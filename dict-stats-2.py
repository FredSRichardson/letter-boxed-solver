...
with open('NASPAZyzzyva342/install/data/words/North-American/NWL2023.txt', 'r', encoding='latin-1') as ifp:
    for line in ifp:
        line = line.strip()
        res = re.match(r'^(\S+)\s.*$', line)
        assert res is not None, f'ERROR: could not parse word from {line}'
        wrd = res.group(1)
        lexicon.add(wrd)

print(f'Number of words: {len(lexicon)}')
            
with open('zyzzyva-clean.txt', 'w') as ofp:
